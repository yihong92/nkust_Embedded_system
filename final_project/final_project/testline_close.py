...
if __name__ == "__main__":
    ...
    # 創建一個 3x3 的結構元素（結構元素的大小可根據需求調整）
    kernel = np.ones((3, 3), np.uint8)
    
    # 進行閉運算：先膨脹再侵蝕
    closedx = binary_erosion(binary_dilation(binaryx, structure=kernel), structure=kernel).astype(np.uint8)
    closedy = binary_erosion(binary_dilation(binaryy, structure=kernel), structure=kernel).astype(np.uint8)
    closedx_normalized = cv2.normalize(closedx, None, 0, 255, cv2.NORM_MINMAX).astype(np.uint8)
    closedy_normalized = cv2.normalize(closedy, None, 0, 255, cv2.NORM_MINMAX).astype(np.uint8)

    # 儲存閉運算後的圖像（可選）
    cv2.imwrite('sobelx_closed.jpg', closedx_normalized)
    cv2.imwrite('sobely_closed.jpg', closedy_normalized)
    
    # 計算閉運算後符合條件的直線數量
    line_countx = count_lines(closedx, offset=22)  # 這裡可以調整offset來過濾短的線條
    line_county = count_lines(closedy, offset=22)
    
    # 計算總的直線數量
    total_line_count = line_countx + line_county
    print(f'Total lines count (after closing and filtering): {total_line_count}')
